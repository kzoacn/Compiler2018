...
global_init:
	...
	mov    rbp, rsp
	sub    rsp, 408
	mov qword [gbl+8*3] ,15000
	mov     rdi, 15001
	call    mallocArray
	mov     qword [gbl+8*42], rax
	mov r8, [gbl+8*42]
	mov qword [gbl+8*5] ,r8
	mov qword [gbl+8*20] ,0
	mov rax,[rsp+8*43]
	leave
	ret
...
tmpVariableimpstzzwjj:
	 db 1," " ,0

tmpVariableslxnahhiid:
	 db 7,"Total: " ,0
